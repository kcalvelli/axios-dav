...
let
  ...
  mcp-dav =
    pkgs.mcp-dav or (
      if inputs ? cairn-dav then
        inputs.cairn-dav.packages.${pkgs.stdenv.hostPlatform.system}.mcp-dav
      else if inputs ? self then
        inputs.self.packages.${pkgs.stdenv.hostPlatform.system}.mcp-dav
      else
        throw "mcp-dav package not found. Add cairn-dav overlay or ensure inputs.cairn-dav is available."
    );
  ...
  vdirsyncerConfig = ''
    # Auto-generated by cairn-dav - do not edit manually
...
in
{
  ...
  config = lib.mkIf enabled {
    ...
    # Systemd user service for vdirsyncer sync (mkForce to override cairn's home/calendar module)
    ...
    # Timer for periodic sync (mkForce to override cairn's home/calendar module)
    ...
    # The actual MCP registration happens in the consuming config (e.g., cairn)
    # by reading this module's output and adding to their MCP server list
    #
    # Example MCP server registration (in cairn home/ai/mcp.nix):
    #   mcp-dav = {
